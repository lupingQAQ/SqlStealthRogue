...
> **让天底下没有难拖的数据库。** · [English docs](README.md)
...
**一把极简、零探测的 SQL/NoSQL 注入脱库工具。** 单文件入口、纯 Python 标准库、零依赖。

SqlStealthRogue 是一把"手术刀"，不是瑞士军刀：在**已知注入点、已知库表列**的前提下，以**零协商流量、零侦察请求**完成高速数据提取。sqlmap 即使指定了 `-D/-T/-C`，仍会发出大量指纹识别、版本探测、权限枚举、WAF 检测、注入技术轮询等"环境探查"请求；SqlStealthRogue 反其道而行——**发出的每一个请求，都是数据提取请求本身。**

> ⚠️ **仅限已授权的安全测试使用。** 对未获书面授权的系统使用本工具属于违法行为，使用者需自行承担全部法律责任。
...
内置：`space2comment` `space2plus` `space2randomblank` `between` `equaltolike` `randomcase` `charencode` `chardoubleencode` `halfversionedmorekeywords` `apostrophemask` `percentage` `unionalltounion` `xforwardedfor`
...
¹ 按真机证据关闭（引擎错误信息不含查询数据或未实现对应函数）² 模板内置未实机验证 ³ 23ai 的 XMLType 报错已不回显数据；旧版本可用 `-A` 手动启用（模板见 `dbms/oracle.py` 注释）
...
```
SqlStealthRogue/
├── SqlStealthRogue.py          # 薄壳入口
└── SqlStealthRogue/
    ├── core.py                 # 引擎：keep-alive 发送器 + 5 种提取算法 + 行迭代 + 配置错误哨兵
    ├── cli.py                  # 参数 / 校验 / 调度
    ├── dbms/                   # 引擎插件：一个文件一个库，丢进去即注册（base 继承链）
    └── tampers/                # WAF 绕过插件：一个文件一个 tamper()，同名覆盖内置
```

**五种提取算法**（按流量从少到多）：`union`（整表 1 请求）→ `error`（每 30~500 字符 1 请求，分块预取）→ `bool`（每字符 8 请求，位并行）→ `time`（每字符 8 请求，刻意串行防打挂目标）→ `prefix`（NoSQL `$regex` 字符集二分）。

**占位符契约**：URL/body 里的 `*` = 注入点；模板里的 `{Q}` 查询、`{F}` 字段、`{P}` 位置、`{N}` 块长、`{K}/{M}` 二分值/位掩码、`{T}` 延时、`{S}` 前缀、`{HH}` 字符类上界。所有模板可用 `-A/-B/--bit-tpl/--time-tpl/-U` 覆盖。

**新增一个数据库 = 在 `dbms/` 加一个几十行的 PROFILE 文件**，零注册代码；兼容库走 `base` 继承（`pgvector` 整个文件只有一行 `base: 'postgres'`）。

设计决策见 [ARCHITECTURE.md](ARCHITECTURE.md)，插件贡献见 [CONTRIBUTING.md](CONTRIBUTING.md)，授权使用政策见 [SECURITY.md](SECURITY.md)。
...
本项目仅供**已授权**的渗透测试、安全研究与教学用途。对任何未经授权的系统进行测试均属违法。开发者不对任何滥用行为承担责任。使用本工具即表示你已获得目标系统所有者的书面授权。
